...
uint256 hashGenesisBlock("0xf2a02df42853e11ee406a22cf94873880099bd9e1a76d057e8bcfd34bbab89ec");
...
const string strMessageMagic = "RitzCoin Signed Message:\n";
...
{
    // RitzCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // RitzCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1462729605;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2146180;

        if (fTestNet)
        {
        	block.nTime    = 1448880557;
            block.nNonce   = 36691;
        }
	    ...
	    assert(block.hashMerkleRoot == uint256("0x37edb44589a21d26a176534c22f84448799631cc1565220000398a67cf2096eb"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xd6, 0xc4, 0xac, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RitzCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RitzCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RitzCoinMiner(CWallet *pwallet)
{
    printf("RitzCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("RitzCoin-miner");
    ...
    try { loop {
        ...
        printf("Running RitzCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RitzCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RitzCoinMiner, pwallet));
}
