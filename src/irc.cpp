...
// Copyright (c) 2011-2012 RitzCoin Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #RitzCoinTEST0\r");
            Send(hSocket, "WHO #RitzCoinTEST0\r");
        } else {
            // randomly join #RitzCoin00-#RitzCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #RitzCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #RitzCoin%02d\r", channel_number).c_str());
        }
